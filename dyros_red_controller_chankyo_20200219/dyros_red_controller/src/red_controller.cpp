...
void RedController::dynamicsThreadLow()
{
    ...
    while (!dc.shutdown && ros::ok())
    {
        ...
        if (task_switch)
        {
            ...
            else if (tc.mode == 10)   // 바닥 고정 - 허리 고정 - 왼손 움직임
            {
                ...
                red_.link_[Left_Hand].rot_desired = red_.link_[Left_Hand].rot_init*DyrosMath::rotateWithX(0) * DyrosMath::rotateWithY(3.141592 / 2.0) * DyrosMath::rotateWithZ(3.141592 / 2.0);  //승범좌
            ...
            else if (tc.mode == 14)  //mode 11  복붙 뒤에 임의로 작성
            {
                wc_.set_contact(red_, 1, 1);
                torque_grav = wc_.gravity_compensation_torque(red_, dc.fixedgravity);
            ...
            else if (tc.mode == 15)  //왼손 제어 코드
            {
                ...
                for(int i = 0; i<3; i++)
                {
                    x_dot_desired(i) = 0.0 + k_pos(i)*error_v(i); // linear velocity ,초기값 0으로 설정
                ...
                // 이부분은 키넥트에서 서버를 통해 값을 받은 다음에 설정/////////////////////전역변수로 선언해야 함-------> q_desired_pre_ = (초기 8개 조인트 좌표 값);
                
                //std::cout << "q_dot_desired_ = " << q_dot_desired_.block(0,0,8,1) / 15.0 << std::endl;
                q_desired_.block(0,0,8,1) = q_dot_desired_.block(0,0,8,1) / dc.dym_hz_chankyo;  //초기 qdot 값을 0으로 설정
